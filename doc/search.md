...
Best-effort precompile returning `(compiled_fn, autotuner)`. Captures Triton's `Autotuner` reference for cache introspection so `_record_autotune_winner` can read winners post-bench. `compile_kernel` exceptions degrade to `(None, None)` with a WARNING and the lazy-compile path inside `benchmark_kernel` takes over (autotune_winner stays empty for that workload). *role* labels postmortem warnings ("Baseline" / "Child"). Called at the baseline (Phase A) and per-iter child (Phase B) precompile sites.

#### `_record_autotune_winner(kernel, autotuner, workloads, definition=None)`

Populates `kernel.autotune_winner` (dict keyed by `workload.uuid`) by computing `_key_tuple_for(...)` per workload and **prefix-matching** against Triton's autotune cache. Prefix matching is required because Triton appends pointer-dtype tags after the user-declared `key=` values (e.g., cache key is `(M, N, K, "torch.float32", "torch.float32", "torch.float32")` but we only compute `(M, N, K)`). Best-effort: failures log a WARNING; skipped when `autotuner is None` or `autotune_keys` is empty.
