...
- **Analytical (optional)** — `_compute_analytical()` derives `AnalyticalMetrics` from `(flops, nbytes, latency_s, HardwareSpec, input_dtypes)`, narrowed to per-iter dynamics only: `achieved_tflops`, `achieved_bandwidth_gb_s`, `pct_peak_compute`, `pct_peak_bandwidth`, `compute_peak_dtype`, `compute_peak_calibration_warning`. Fails closed with `ProfilerError` on zero-latency, non-positive `nbytes`, negative `flops`, or zero-peak hardware (the orchestrator marks the branch DEAD_END). `profile_kernel` skips the analytical computation entirely when `nbytes == 0` (no byte count derivable for this iteration) and the resulting `ProfilingResult.analytical` is `None`; NCU still runs. Run-level invariants `arithmetic_intensity` and `ridge_point` (both in MACs/byte) live on `RooflineResult` (`src/eval/roofline.py`); the Reviewer prompt builder reads AI / ridge from `RooflineResult` and achieved_* / pct_peak_* from `AnalyticalMetrics`.

  **Dtype-aware compute peak (2026-05-28).** `pct_peak.compute` divides `achieved_tflops` by the hardware peak for the **lowest-precision input dtype** seen at bench time (BF16 inputs → `peak_flops_bf16`, FP32 → `peak_flops_fp32`, etc.). `_pick_compute_peak(input_dtypes, hardware_spec)` is the chooser; callers (`bench_worker`, orchestrator baseline-profile, `pipeline.report`) gather dtypes via `_collect_input_dtypes(input_generator(0))` and forward them as `profile_kernel(..., input_dtypes=...)`. The chosen dtype rides on `AnalyticalMetrics.compute_peak_dtype` — values: `bf16` / `fp16` / `tf32` / `fp8` / `nvfp4` / `fp32` / `fp32_fallback`. The `fp32_fallback` label plus `compute_peak_calibration_warning=True` indicate the heuristic cascaded into FP32 because the preferred dtype peak was missing or zero (placeholder hardware, undetectable dtype, no tensors visible). Renderers in `src/pipeline/report.py`, `src/agents/reviewer.py`, and `src/search/orchestrator.py` append the label as `[<dtype>]` to the `compute X%` field — fixes the >100% pct_peak overflow on tensor-core kernels (`run_20260527T150715_440277Z` showed BF16 matmul at `compute 172.5%` against the wrong FP32 denominator). Spec: `doc/specs/2026-05-28-pct-peak-dtype-and-warmup-traceback-design.md`.
...
| `warmup_failed` | `config_idx`, `exc_class`, `exc_traceback` | Per-`Config` `autotuner.fn.warmup(...)` raised; sweep continues, this `Config` is excluded. `exc_traceback` is `traceback.format_exception(...)` tail-sliced to 2048 chars (Triton annotates `at <line>:<col>:` at the END of the chain, so tail-slicing preserves the source annotation that the prior 160-char `exc_msg` head-slice was cutting — `run_20260527T150715_440277Z`). | Per-`Config` |
