...
The Reviewer can be promoted from the default submit-only shape into a bounded multi-turn shape that lets the LLM pull additional raw NCU metrics on demand, instead of receiving the full `raw_metrics` dict up front in the curated profiling summary.
...
- **What activating it changes**: tool registration grows from `[submit_review]` to `[submit_review, query_metric]`, the SDK turn budget goes from `max_turns=4` to `max_turns=6` (room for a fetch-then-submit round-trip on top of the existing one in-band validation retry), and `build_user_prompt()` appends a `## Available raw metrics (queryable)` section listing the iteration's `ProfilingResult.raw_metrics` keys as a menu the LLM can pick from.
- **`query_metric(names: list[str]) -> dict[str, str]`** — argument is the list of metric names to fetch; return value is a name→value map. Values are stringified floats for known names; unknown names map to the sentinel `"[unknown]"`; if the iteration's `raw_metrics` is empty the entire response is `{name: "[no data]"}` for every requested name. The LLM is expected to read the curated summary first and only `query_metric` for genuinely missing signals before calling `submit_review`.
