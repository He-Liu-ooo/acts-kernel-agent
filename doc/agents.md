...
The Reviewer can be promoted from a single-call submit shape into a bounded multi-turn shape that lets the LLM pull additional raw NCU metrics on demand, instead of receiving the full `raw_metrics` dict up front in the curated profiling summary.

- **Flag**: `ACTSConfig.reviewer_metric_queries: bool` — defaults to `False`. Lives in INI under `[search]` as `reviewer_metric_queries = false`. Off by default so the existing single-call shape stays the canonical path; flipping it on is the only change needed to enable on-demand fetches.
