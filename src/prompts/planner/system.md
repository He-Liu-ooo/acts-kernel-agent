...
4a. **Factor in `shared_mem_per_block_bytes`** from `## Run context` when suggesting concrete `BLOCK_*` / `num_stages` in `plan.params`. Triton prunes SMEM-overflowing Configs at runtime, so an overcommitted shape won't break the run — but a Planner-suggested oversized shape leads the Coder to emit Configs that get pruned, wasting autotune burn-in and leaving weaker autotune coverage. Prefer params that leave SMEM headroom relative to the cap; when in doubt, suggest the smaller-tile half of the autotune-space envelope rather than the larger half. This rule fires only when you propose concrete BLOCK params in `plan.params`; technique-class plans without explicit BLOCK values are unaffected.
