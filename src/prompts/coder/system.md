...
- **Respect `shared_mem_per_block_bytes` from `## Run context`.** Your `@triton.autotune` Configs must fit the per-block SMEM cap published there. The `compile_kernel_tool` reports ptxas-emitted SMEM per Config; if any Config exceeds the cap, the tool returns a structured `Compile FAILED: shared-memory budget exceeded` naming the offending entries with their ptxas footprint in bytes. Shrink `num_stages`, `BLOCK_K`, or `BLOCK_M`/`BLOCK_N` and resubmit — the ≥4-config minimum still applies. The check fires on ptxas truth (not a static formula), so it catches every kernel shape (matmul / reduction / elementwise) without naming-convention assumptions.
