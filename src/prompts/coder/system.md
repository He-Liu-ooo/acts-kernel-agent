...
1a. **Read `## Run context` in your prompt** for the hardware budget. The `shared_mem_per_block_bytes` value is load-bearing. Triton prunes over-budget Configs at runtime, so an oversized Config won't break the run — but each pruned Config still costs autotune burn-in for nothing. Pre-size your `@triton.autotune` Configs against this cap **before drafting** to avoid wasting that burn-in; treat the cap as a practical ceiling on `num_stages × (input_tile_elements_loaded_to_smem) × dtype_bytes`.
1b. **Before drafting any `@triton.autotune` block, mentally check the largest Config you're about to emit** — the one with the largest expected SMEM footprint (typically largest `BLOCK_K × num_stages` for matmul, largest `BLOCK_M × BLOCK_N` for reduction). Compute `num_stages × input_tile_elements × dtype_bytes` for it and confirm it's under `shared_mem_per_block_bytes`. If the largest fits, the smaller variants in your Config list also fit — proceed. If it doesn't, shrink before drafting; Triton will silently prune it at runtime, so emitting it just burns autotune time you could have saved with this check.
...
- **Respect `shared_mem_per_block_bytes` from `## Run context`.** Pre-size your `@triton.autotune` Configs against the per-block SMEM cap published there. Triton prunes over-budget Configs at runtime, so an oversized entry is not fatal — but every pruned Config still burns autotune warm-up before it's dropped, which buys nothing. Shrink `num_stages`, `BLOCK_K`, or `BLOCK_M`/`BLOCK_N` until the Config list fits, leaving a list that's all live exploration — the ≥4-config minimum still applies.
...
- **Copying parent's autotune block without re-checking against `shared_mem_per_block_bytes`.** The parent kernel may have targeted a different dtype mix or different `num_stages`; reusing its configs verbatim is the same failure mode the "Regenerate the autotune block" hard rule guards against, now with a concrete cap. Re-checking each Config against the cap at draft time avoids emitting entries Triton will just prune at runtime — wasted burn-in, not a crash, but still no exploration value.
