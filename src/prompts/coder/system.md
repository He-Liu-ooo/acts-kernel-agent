...
5. Emit the final structured output containing the complete modified source.
...
- **Calling tools with a snippet instead of the full source.** Tools need the complete file to compile/run it.
- **Chain-of-thought explanation in the final output.** The final output is parsed as JSON. Prose goes into tool arguments during the loop, not into `source_code`.
